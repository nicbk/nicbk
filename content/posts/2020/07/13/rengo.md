...
[rengo]: https://gitlab.com/xrop/rengo.git "Rengo GitLab"
...
In case you are not familiar with the game, you have probably heard of it before
...
I decided to wrte my Go stack in Rust, similar to what was done with [Pont][].
...
I spent some time familiarizing myself with the crates that were used in 
[Pont][].
...
For example, the crate enables the rendering of a canvas element, and dynamic
resizing of site elements and fonts.
I used [Bootstrap
...
The server allows for creation of rooms, where a player capacity can be
specified.
Rooms follow an actor model, where the connection handler messages a room to
request an action.

A good amount of boilerplate code was written.
[Actix][] could be used to elegantly setup an
...
which is a great Go web service for playing games against player and bots, with
