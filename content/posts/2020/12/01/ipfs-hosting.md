...
This functions similarly to BitTorrent, where bits of data can come from
different nodes simultaneously.
The transfer of data can be much more efficient than a traditional star topology
network (including CDNs), because data can be retrieved in smaller parts from many
nodes, instead of a full large blob coming from a single server, and from
potentially further away.
My first concern was scalability; however, the compromise looks quite
reasonable.
...
Data which has been cached by the node is frequently garbage collected.
...
Web browsers by default use DNS and Internet Protocol for resolving addresses and
finding servers.
...
will be sharing the same content identifiers as me.
